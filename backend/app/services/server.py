...
def pinecone_text_operations(total_text:list, namespace:str):
...
def store_data(pdf_contents: str, qNa: dict, links: list):
    ...
    if pdf_contents:
        ...
        pdf_status=pinecone_text_operations(total_text, config['pinecone']['pdf_namespace'])
    ...
    # for links
    if links:
        scraped_data=site_scraper.scrape_sites(links)  # receives a list of string
        split_text=text_splitter.extract_text_from_string(scraped_data, chunk_size, chunk_overlap)
        links_status=pinecone_text_operations(split_text, config['pinecone']['link_namespace'])

    return {"pdf": pdf_status, "qNa": qna_status, "links": links_status}


def query_results(query:str, sources:dict, text: str, links) -> str:
    valid_namespaces=[]
    ...
    llm_response=openai_response.openai_response(query, pinecone_results, text, links)
